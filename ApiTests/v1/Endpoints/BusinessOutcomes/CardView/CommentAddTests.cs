...
namespace ApiTests.v1.Endpoints.BusinessOutcomes.CardView
{
    ...
    public class CommentAddTests : BaseV1Test
    {
        
        [TestMethod]
        [TestCategory("CompanyAdmin"), TestCategory("SiteAdmin"), TestCategory("OrgLeader")]
        [TestCategory("TeamAdmin"), TestCategory("BLAdmin"), TestCategory("Member")]
        public async Task BusinessOutcomes_Comment_Put_Ok()
        {
            // arrange
            ...
            var keyResultId = boResponse.Dto.KeyResults[0].BusinessOutcomeId;

            // act
            bo = BusinessOutcomesFactory.GetValidPutBusinessOutcomeWithComment(Company, businessOutcomeId, keyResultId, User);
            ...
            Assert.AreEqual(bo.Comments.Count, boResponse.Dto.Comments.Count, "Comment's count doesn't match");
            Assert.IsNotNull(bo.Comments[0].ItemId, "Comment ItemId is null");
            Assert.AreEqual(bo.Comments[0].Content, boResponse.Dto.Comments[0].Content, "Comment's Content doesn't match");
            Assert.AreEqual(bo.Comments[0].Commenter, boResponse.Dto.Comments[0].Commenter, "Comment's Commenter doesn't match");
            Assert.IsFalse(bo.Comments[0].IsDeleted, "Comment's IsDeleted is true");
            Assert.IsNotNull(bo.Comments[0].Uid, "Comment uid is null");
        }
    }
}