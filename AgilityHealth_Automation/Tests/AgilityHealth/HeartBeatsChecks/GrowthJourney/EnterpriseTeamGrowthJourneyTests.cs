...
namespace AgilityHealth_Automation.Tests.AgilityHealth.HeartBeatsChecks.GrowthJourney
{
    ...
    public class EnterpriseTeamGrowthJourneyTests : BaseTest
    {
        ...
        public void VerifyEnterpriseTeamGrowthJourneyPageNavigationInProd(string env)
        {
            if (env == "eu" || env == "nscorp" || env == "raytheon" || env == "usaf") { return; }
            var growthJourneyPage = new GrowthJourneyPage(Driver, Log);
            if (!NavigateToEnterpriseTeamGrowthJourneyPage(env)) return;
            TakeFullPageScreenShot($"{FileUtil.GetBasePath()}Resources\\Screenshots\\HeartBeatScreenshots\\{env}\\EnterpriseGrowthJourney.png");
            Assert.IsTrue(growthJourneyPage.IsGrowthJourneyTabPresent(), $"Enterprise growth journey tab is not present for the '{env}' environment.");
        }
        ...
        public bool NavigateToEnterpriseTeamGrowthJourneyPage(string env)
        {
            Log.Info($"Login to environment {env} and Go to 'Enterprise-Team' growth journey tab");
            ...
            Log.Info($"Login to {env}, Navigate to 'Team dashboard' page and click on first 'Enterprise-Team' from list.");
            ...
            {
                Log.Info($"No enterprise-teams in this environment {env}");
                return false;
            }
