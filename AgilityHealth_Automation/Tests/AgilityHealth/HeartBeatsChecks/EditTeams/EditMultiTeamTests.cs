...
namespace AgilityHealth_Automation.Tests.AgilityHealth.HeartBeatsChecks.EditTeams
{
    ...
    public class EditMultiTeamTests : BaseTest
    {
        ...
        public void VerifyThatEditMultiTeamAllSteppersLoadedSuccessfully(string env)
        {
            ...
            var teamNames = teamDashboardPage.GetAllTeamsNames("Multi-Team");
            ...
            Assert.AreEqual("Edit " + firstTeamName, editProfileBasePage.GetTeamProfilePageTitle(), $"The Edit Team Profile tab header title does not contain the expected team name. Actual: '{actualTeamProfileTabHeaderTitle}' - {env}");
            ...
            Log.Info("Verify that user is navigated on the Stakeholders tab successfully");
            editTeamBasePage.GoToStakeHoldersTab();            
            Assert.AreEqual("Time to edit some stakeholders.", memberCommonPage.GetPageHeaderTitle(), $"The Stakeholders tab does not loaded successfully - {env}");

            Log.Info("Verify that user is navigated on the Metrics tab successfully");
            editTeamBasePage.IsMetricsTabPresent();
            editTeamBasePage.GoToMetricsTab();
            var expectedMetricsTabHeaderTitle = "A high performing healthy team uses both subjective measures (Team Health Assessment) and hard metrics to understand their performance. Enter the data below to unlock your metrics Dashboard!";
            Assert.AreEqual(expectedMetricsTabHeaderTitle, memberCommonPage.GetPageHeaderTitle(), $"The Metrics tab does not loaded successfully - {env}");
        }
    }
}