...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Settings.ManageIntegrations.JiraCloud
{
    ...
    public class JiraCloudTeamLinkUnlinkTests : BaseTest
    {
        ...
        public void VerifyLinkUnlinkTeamWithJiraBoard()
        {
            ...
            linkTeamPage.UnlinkAlreadyLinkedTeam(JiraCloudData.JiraBoard);
            linkTeamPage.LinKTeam(JiraCloudData.JiraProjectName, JiraCloudData.JiraBoard, _teamResponse.Name);

            Assert.AreEqual(JiraCloudData.JiraProjectName, linkTeamPage.GetLinkedJiraProjectName("Static Jira Test Automation Project - Do Not Touch"), "Project name does not match");
            Assert.AreEqual(JiraCloudData.JiraBoard, linkTeamPage.GetLinkedJiraBoardName(JiraCloudData.JiraBoard), $"The linked board name is not matched with the selected board {JiraCloudData.JiraBoard}.");
            ...
            Log.Info($"Verify that the linked team - {_teamResponse.Name} and Jira board - {JiraCloudData.JiraBoard} are not available in the 'Select Team' and 'Select Board' dropdown lists respectively.");
            ...
            Assert.IsFalse(listOfBoard.Contains(JiraCloudData.JiraBoard), $"Jira Board name {JiraCloudData.JiraBoard} is still present in the 'Select JiraBoard' list");
            ...
            Assert.AreEqual(JiraCloudData.JiraBoard, teamMetricsPage.GetJiraBoardName(_team.Name), "The 'Jira Board' name does not match.");
            ...
            Log.Info($"Unlink the Team {_teamResponse.Name} linked with {JiraCloudData.JiraBoard} Board.");
            linkTeamPage.UnlinkAlreadyLinkedTeam(JiraCloudData.JiraBoard);
            Assert.IsFalse(linkTeamPage.IsUnlinkButtonEnabled(), "The Unlink button is not present");

            Log.Info($"Verify that the unlinked Team {_teamResponse.Name} and Jira Board {JiraCloudData.JiraBoard} are available in the 'Select Team' and 'Select Board' dropdown lists respectively.");
            linkTeamPage.SelectJiraProject(JiraCloudData.JiraProjectName);
