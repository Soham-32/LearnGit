...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.IndividualAssessment.BatchEdit
{
    ...
    public class BatchEditHeaderDeleteButtonTest1 : IndividualAssessmentBaseTest
    {
        ...
        public void BatchEdit_Header_TrashButton_Cancel()
        {
            ...
            batchEditAssessmentPage.ClickPopupCancelButton();

            //assert
            Log.Info("Assert: Verify cancel button keeps user on batch edit page");
            Assert.IsTrue(batchEditAssessmentPage.IsBatchEditPagePresent(), "User should still be on Batch Edit Page.");
        }
    }
}