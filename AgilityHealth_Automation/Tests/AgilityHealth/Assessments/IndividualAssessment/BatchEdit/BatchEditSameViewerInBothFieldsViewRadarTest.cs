...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.IndividualAssessment.BatchEdit
{
    ...
    public class BatchEditSameViewerInBothFieldsViewRadarTest : IndividualAssessmentBaseTest
    {
        ...
        public void BatchEdit_Add_SameViewerInBothFields_ViewRadar()
        {
            ...
            Log.Info($"Assert: Verify viewer {_individualAssessViewer1.Username} able to access individual and batch radar");
            Assert.IsTrue(individualAssessmentDashboard.IsAssessmentPresent(
                    $"{_assessment.AssessmentName} - Roll up"),
                $"Individual Roll up doesn't exists with name : {_assessment.AssessmentName}");
            Assert.IsTrue(individualAssessmentDashboard.IsAssessmentPresent(
                    $"{_assessment.AssessmentName} - {_team.Members[0].FullName()}"),
                $"Individual radar doesn't exists with name : {_assessment.AssessmentName}");
        }
    }
}