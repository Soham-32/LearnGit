...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.IndividualAssessment.BatchEdit
{
    ...
    public class BatchEditEditParticipantGroupsTests : IndividualAssessmentBaseTest
    {
        ...
        public void BatchEdit_EditParticipantGroups()
        {
            ...
            batchEditParticipantReviewerPage.UpdateParticipantGroup(participantGroups);

            Log.Info("Assert: Verify that edited participant with participant groups displays in participants list");
            Assert.IsTrue(batchEditParticipantReviewerPage.DoesParticipantDisplay($"{_assessmentResponse.Members[0].FirstName} {_assessmentResponse.Members[0].LastName}", participantEmail, participantGroups),
