...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.IndividualAssessment.Edit
{
    ...
    public class EditIndividualAssessmentVerifyReviewerRoleFromAssessmentTest2 : IndividualAssessmentBaseTest
    {
        ...
        public void IndividualAssessment_Edit_ReviewerRole_Multiple()
        {
            ...
            var roleList = new List<string> {"Reviewer", "Peer"};
            ...
            var actualRoles = iaEditPage.GetReviewerRole().Split(',').Select(r => r.Trim()).ToList();

            Log.Info("Verify reviewer role from assessment shows");
            Assert.That.ListsAreEqual(roleList, actualRoles, $"Expected roles and actual roles shown on the individual assessment edit page do not match");
        }
    }
}