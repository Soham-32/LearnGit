...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.PulseAssessmentV2.MultiTeamLevel
{
    ...
    public class PulseAssessmentV2AddMemberVerificationMultiTeamLevelTests : PulseV2BaseTest
    {
        ...
        private static TeamResponse _teamClosedPulse;
        ...
        public static void ClassSetup(TestContext _)
        {
            try
            {
                ...
                _teamClosedPulse = _setupApi.CreateTeam(teamClosedPulse).GetAwaiter().GetResult();
                _teamIdClosedPulse = _setupApi.GetCompanyHierarchy(Company.Id).GetTeamByName(_teamClosedPulse.Name).TeamId;
                _teamWithMembersOfClosedPulse = _setupApi.GetTeamWithTeamMemberResponse(_teamIdClosedPulse);
                ...
                _setupApi.AddSubteams(multiTeamResponse.Uid, new List<Guid> { _teamWithDraftPulse.Uid, _teamWithActivePulse.Uid, _teamWithPendingPulse.Uid, _teamClosedPulse.Uid }).GetAwaiter()
                    .GetResult();
                ...
                _multiHierarchyResponses = _setupApi.GetCompanyHierarchy(Company.Id).GetTeamByName(multiTeamResponse.Name);
                _setupApi.GetTeamWithTeamMemberResponse(_multiHierarchyResponses.TeamId);
                var questionDetailsResponse = GetQuestions(_multiHierarchyResponses.TeamId);
                ...
                _draftPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _teamWithMembersOfDraftPulse, _multiHierarchyResponses.TeamId, false, period: AssessmentPeriod.TwentyFourHours);

                _activePulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _teamWithMembersOfActivePulse, _multiHierarchyResponses.TeamId, period: AssessmentPeriod.OneWeek);

                var futureStartDate = DateTime.UtcNow.AddDays(1);
                _pendingPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _teamWithMembersOfPendingPulse, _multiHierarchyResponses.TeamId, true, null, AssessmentPeriod.TwentyFourHours, RepeatIntervalId.Never, futureStartDate);

                _closedPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _teamWithMembersOfClosedPulse, _multiHierarchyResponses.TeamId, period: AssessmentPeriod.SeventyTwoHours);
        ...
        public void PulseV2_AddMember_DraftPulse_MultiTeamLevel()
        {
            VerifySetup(_classInitFailed);
            AddPulseAssessment(_draftPulseRequest);
            VerifyPulseScenario(_draftPulseRequest, _teamWithDraftPulse, _teamIdWithDraftPulse, _teamWithMembersOfDraftPulse);
        }

        [TestMethod]
        [TestCategory("CompanyAdmin")]
        [Description("Verifies adding a team member in Active pulse scenarios.")]
        public void PulseV2_AddMember_ActivePulse_MultiTeamLevel()
        {
            VerifySetup(_classInitFailed);
        ...
        public void PulseV2_AddMember_PendingPulse_MultiTeamLevel()
        {
            VerifySetup(_classInitFailed);
        ...
        public void PulseV2_AddMember_ClosedPulse_MultiTeamLevel()
        {
            VerifySetup(_classInitFailed);
            AddPulseAssessment(_closedPulseRequest);
            VerifyPulseScenario(_closedPulseRequest, _teamClosedPulse, _teamIdClosedPulse, _teamWithMembersOfClosedPulse);
        }
        ...
        private void VerifyPulseScenario(SavePulseAssessmentV2Request pulseRequest, TeamResponse team, int teamId, IList<TeamV2Response> teamWithMembers)
        {
            ...
            if (pulseRequest == _closedPulseRequest)
            {
                ...
                {
                    Assert.IsTrue(GmailUtil.DoesMemberEmailExist(SharedConstants.PulseEmailSubject(_teamClosedPulse.Name), participant.Email, "Inbox", "", 360), $"Could not find email with subject <{SharedConstants.PulseEmailSubject(_teamClosedPulse.Name)}> sent to {participant.Email}");
                    CompletePulseSurvey(TestContext, _filteredQuestions, _teamClosedPulse.Name, participant.Email);
                }
            }
            ...
            mtEtDashboardPage.NavigateToPage(_multiHierarchyResponses.TeamId);
            ...
            var pariciapntEmail = teamWithMembers.First().SelectedParticipants.Select(a => a.Email).ToList();
            var combinedList = teamMemberEmailList.Concat(pariciapntEmail).ToList();
