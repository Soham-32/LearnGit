...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.PulseAssessmentV2.MultiTeamLevel
{
    ...
    public class PulseAssessmentV2EveryMultiTeamLevelTests : PulseV2BaseTest
    {
        ...
        public static void ClassSetup(TestContext _)
        {
            try
            {
                ...
                var multiHierarchyResponses = setupApi.GetCompanyHierarchy(Company.Id).GetTeamByName(multiTeam.Name);
                var questionDetailsResponse = GetQuestions(multiHierarchyResponses.TeamId);
                ...
                _activePulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, teamMembers, multiHierarchyResponses.TeamId, period: AssessmentPeriod.OneWeek);

                _closedPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, team1Members, multiHierarchyResponses.TeamId, period: AssessmentPeriod.SeventyTwoHours);

                var futureStartDate = DateTime.UtcNow.AddDays(1);
                _pendingPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, teamMembers, multiHierarchyResponses.TeamId, true, null, AssessmentPeriod.TwentyFourHours, RepeatIntervalId.Never, futureStartDate);

                _draftPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, teamMembers, multiHierarchyResponses.TeamId, false, period: AssessmentPeriod.TwoWeeks);
        ...
        [Description("Verifies that a draft, active, pending and closed Pulse Assessment are created at the Multi Team Level and its appears at different levels in the company hierarchy, including N-Tier, Enterprise, Multi-Team, and Team levels.")]
        public void PulseV2_ActiveDraftPendingClose_Pulse_At_TeamLevel_And_VerifyAtEveryLevel()
        {
