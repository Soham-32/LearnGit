...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.PulseAssessmentV2.EnterpriseLevel
{
    ...
    public class PulseAssessmentV2RemoveTeamMemberFromSelectedTeamTests : PulseV2BaseTest
    {
        ...
        public static void ClassSetup(TestContext _)
        {
            try
            {
                ...
                _enterpriseHierarchyResponses = setupApi.GetCompanyHierarchy(Company.Id).GetTeamByName(enterpriseTeamResponse.Name);
                ...
                _draftPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _team1WithMembers, _enterpriseHierarchyResponses.TeamId, false, period: AssessmentPeriod.TwentyFourHours);

                // Pulse request for active pulse
                _activePulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _team2WithMembers, _enterpriseHierarchyResponses.TeamId, period: AssessmentPeriod.OneWeek);

                // Pulse request for pending pulse
                var futureStartDate = DateTime.UtcNow.AddDays(1);
                _pendingPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _team3WithMembers, _enterpriseHierarchyResponses.TeamId, true, null, AssessmentPeriod.TwentyFourHours, RepeatIntervalId.Never, futureStartDate);

                //Pulse request for Closed Pulse
                _closedPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, _team4WithMembers, _enterpriseHierarchyResponses.TeamId, period: AssessmentPeriod.SeventyTwoHours);
        ...
        public void PulseV2_Draft_Pulse_At_EtLevel_And_RemoveTeamMemberFromSelectedTeam()
        {
        ...
        [TestCategory("CompanyAdmin")]
        [Description("Verifies removing a team member from the Active pulse who has filled survey and not filled survey yet.")]
        public void PulseV2_Active_Pulse_At_EtLevel_And_RemoveTeamMemberFromSelectedTeam()
        {
            VerifySetup(_classInitFailed);
        ...
        [TestCategory("CompanyAdmin")]
        [Description("Verifies removing a team member from the Active pulse who has filled survey and not filled survey yet.")]
        public void PulseV2_Pending_Pulse_At_EtLevel_And_RemoveTeamMemberFromSelectedTeam()
        {
            VerifySetup(_classInitFailed);
        ...
        [TestCategory("CompanyAdmin")]
        [Description("Verifies removing a team member from the Active pulse who has filled survey and not filled survey yet.")]
        public void PulseV2_Closed_Pulse_At_EtLevel_And_RemoveTeamMemberFromSelectedTeam()
        {
            VerifySetup(_classInitFailed);
            ...
            IList<TeamV2Response> teamWithMembers)
        {
            ...
            Log.Info("Navigate to Enterprise dashboard, Edit pulse and verify team member details before removing team member");
            ...
            Log.Info("Navigate to Enterprise dashboard, go to created pulse and verify removed member");
            mtEtDashboardPage.NavigateToPage(_enterpriseHierarchyResponses.TeamId, true);
            teamAssessmentDashboardPage.ClickOnMtEtToggleButton();
            if (pulseRequest == _draftPulseRequest || pulseRequest == _pendingPulseRequest)
            {
