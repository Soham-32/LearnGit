...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Assessments.PulseAssessmentV2.NTierLevel
{
    ...
    public class PulseAssessmentNTierLevelEveryTeamLevelTests : PulseV2BaseTest
    {
        ...
        public static void ClassSetup(TestContext _)
        {
            try
            {
                ...
                var ntierHierarchyResponses = setupApi.GetCompanyHierarchy(Company.Id).GetTeamByName(nTierTeamName);
                var questionDetailsResponse = GetQuestions(ntierHierarchyResponses.TeamId);
                ...
                _activePulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, teamMembers, ntierHierarchyResponses.TeamId, period: AssessmentPeriod.OneWeek);

                _closedPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, team1Members, ntierHierarchyResponses.TeamId, period: AssessmentPeriod.SeventyTwoHours);

                var futureStartDate = DateTime.UtcNow.AddDays(1);
                _pendingPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, teamMembers, ntierHierarchyResponses.TeamId, true, null, AssessmentPeriod.TwentyFourHours, RepeatIntervalId.Never, futureStartDate);

                _draftPulseRequest = PulseV2Factory.PulseAssessmentV2AddRequest(_filteredQuestions, teamMembers, ntierHierarchyResponses.TeamId, false, period: AssessmentPeriod.TwoWeeks);
        ...
        [Description("Verifies that a Active, draft, Pending and closed Pulse Assessment is created at the N-Tier Team Level and its appears at different levels in the company hierarchy, including Enterprise, Multi-Team, and Team levels.")]
        public void PulseV2_ActiveDraftPendingClose_Pulse_At_NTierLevel_And_VerifyAtEveryLevel()
        {
            ...
            Log.Info("Navigate to N-Tier assessment dashboard dashboard and verify pulse details");
            ...
            Assert.IsTrue(teamAssessmentDashboardPage.IsAssessmentEditIconDisplayed(activePulseName), $"Edit icon is not displayed for {activePulseName} Pulse Assessment");
            ...
            Assert.IsTrue(teamAssessmentDashboardPage.IsAssessmentEditIconDisplayed(draftPulseName), $"Edit icon is not displayed for {draftPulseName} Pulse Assessment");
            ...
            Assert.IsTrue(teamAssessmentDashboardPage.IsAssessmentEditIconDisplayed(pendingPulseName), $"Edit icon is not displayed for {pendingPulseName} Pulse Assessment");
            ...
            Assert.IsTrue(teamAssessmentDashboardPage.IsAssessmentEditIconDisplayed(closedPulseName),
                $"Edit icon is no displayed for {closedPulseName} Pulse Assessment");
            ...
            Assert.IsTrue(teamAssessmentDashboardPage.IsPulseStatusDisplayed(closedPulseName, "Closed"), $"{closedPulseName} does not exist on n-tier level");
            Assert.IsFalse(teamAssessmentDashboardPage.IsAssessmentEditIconDisplayed(closedPulseName), "Edit icon isdisplayed on Pulse Assessment");
