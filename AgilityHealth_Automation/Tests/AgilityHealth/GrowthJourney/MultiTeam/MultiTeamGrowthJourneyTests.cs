    ...
    public class MultiTeamGrowthJourneyTests : BaseTest
    {
        ...
        public static void ClassSetup(TestContext _)
        {
            try
            {
                ...
                _multiTeam = _setup.GetCompanyHierarchy(Company.Id)
        ...
        public void MultiTeam_GrowthJourney_ExportToPDF()
        {
            ...
            Assert.IsTrue(FileUtil.IsFileDownloaded(filename),
                $"{filename} PDF isn't downloaded successfully");
        ...
        public void MultiTeam_GrowthJourney_VerifyFilterFunctionality()
        {
            VerifySetup(_classInitFailed);
            ...
            Assert.AreEqual("Quarterly", growthJourney.GetSelectedCompareTypeFromFilter(), "Compare doesn't match");
            Assert.IsTrue(growthJourney.IsFilterItemCheckboxSelectedFromFilter(expectedQuarter), $"{expectedQuarter} checkbox isn't selected");
            growthJourney.SwitchToCompareRadarView();

            var colorHex = growthJourney.GetFilterItemColorFromFilter(expectedQuarter);
            foreach (var comp in Constants.TeamHealth2CompentenciesLableForMember.Where(c => !c.Equals("Response to Change")))
            {
                //Verifying whether there is only one avg line or not
                var dotCount = growthJourney.Radar_GetDotValue(colorHex, comp).Count;
                Assert.AreEqual(1, dotCount,
                    $"The validation of Competency '{comp}' failed. Expected: <1>. Actual: <{dotCount}>");


                //Verify avg value (CANT DO AS OF NOW,NEED TO WORK WITH ELKHAIR FOR PROVIDING SUPPORT)
            ...
            Assert.IsTrue(growthJourney.IsFilterItemCheckboxSelectedFromFilter(expectedMonth),
                $"{expectedMonth} checkbox isn't selected");
            growthJourney.SwitchToCompareRadarView();
            ...
            Assert.IsTrue(growthJourney.IsFilterItemCheckboxSelectedFromFilter(expectedBiAnnualTime),
                $"{expectedBiAnnualTime} checkbox isn't selected");
            growthJourney.SwitchToCompareRadarView();
            ...
            {
                //Verifying whether there is only one avg line or not
                Assert.AreEqual(0, growthJourney.Radar_GetDotValue(colorHex, comp).Count, $"Competency: <{comp}> dot count doesn't match");
            }

            growthJourney.SelectCompareTypeFromFilter("Annually");
            radarPage.Filter_OpenFilterSidebar();
            Assert.IsTrue(growthJourney.IsFilterItemCheckboxSelectedFromFilter(expectedAnnualTime), $"{expectedAnnualTime} checkbox isn't selected");
            growthJourney.SwitchToCompareRadarView();
        ...
        [TestCategory("TeamAdmin"), TestCategory("CompanyAdmin"), TestCategory("BLAdmin")]
        public void MultiTeam_GrowthJourney_VerifyCampaignsFilter()
        {
            ...
            Log.Info($"Navigate to the 'Growth Journey' page of the team - '{_multiTeam.Name}'. and verify that campaign- {campaignName} is checked by default in the 'Compare Type' dropdown"); radarPage.NavigateToGrowthJourney(_multiTeam.TeamId, _radar.Id, TeamType.MultiTeam);
            radarPage.NavigateToGrowthJourney(_multiTeam.TeamId, _radar.Id, TeamType.MultiTeam);
            radarPage.Filter_OpenFilterSidebar();
            growthJourney.SelectCompareTypeFromFilter("Campaigns");
            radarPage.Filter_OpenFilterSidebar();
            Assert.IsTrue(growthJourney.IsFilterItemCheckboxSelectedFromFilter(campaignName), $"{campaignName} checkbox isn't selected");
            Assert.AreEqual(campaignName, growthJourney.GetCampaignName(), $"{campaignName} is not present ");

            Log.Info($"Verify that '{campaignName}' is selected by default and also present in the  'Compare Radar Analysis' column");
            radarPage.Filter_OpenFilterSidebar();
            ...
            Log.Info($"Verify that unchecked assessment {campaignName} is not present on the 'Compare Radar Analysis column'");
            radarPage.Filter_OpenFilterSidebar();
            ...
            radarPage.Filter_OpenFilterSidebar();
            Assert.IsTrue(growthJourney.IsItemPresentInColumnList(campaignName), $"{campaignName} is still present in the 'Compare Campaign' column");
        }
        ...
        [TestCategory("DownloadPDF")]
        [TestCategory("TeamAdmin"), TestCategory("CompanyAdmin"), TestCategory("OrgLeader"), TestCategory("BLAdmin")]
        public void MultiTeam_GrowthJourney_Campaigns_ExportToPDF()
        {
            VerifySetup(_classInitFailed);
            ...
            radarPage.NavigateToGrowthJourney(_multiTeam.TeamId, radar.Id, TeamType.MultiTeam);
