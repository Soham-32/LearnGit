...
namespace AgilityHealth_Automation.Tests.AgilityHealth.Company.Edit
{
    ...
    public class CompanySecurityEditTests2 : CompanyEditBaseTest
    {
        ...
        public void Company_UpdateSecurity_Cancel()
        {
            ...
            editSecurityPage.Header.ClickCancelButton();
            companyDashboardPage.WaitUntilLoaded();
            ...
            Assert.AreEqual(_companyRequest.SessionTimeout, actualSecurity.SessionTimeout, 
                "SessionTimeout does not match.");
            Assert.AreEqual(_companyRequest.MaxSessionLength, actualSecurity.MaxSessionLength, 
                "MaxSessionLength does not match.");
            Assert.AreEqual(_companyRequest.ForcePasswordUpdate, actualSecurity.ForcePasswordUpdate, 
                "ForcePasswordUpdate does not match.");
            Assert.AreEqual(_companyRequest.RequireSecurityQuestions, 
                actualSecurity.RequireSecurityQuestions, "RequireSecurityQuestions does not match.");
            Assert.AreEqual(_companyRequest.TwoFactorCompanyAdmin, actualSecurity.TwoFactorCompanyAdmin, 
                "TwoFactorCompanyAdmin does not match.");
            Assert.AreEqual(_companyRequest.TwoFactorOrgLeader, actualSecurity.TwoFactorOrgLeader, 
                "TwoFactorOrgLeader does not match.");
            Assert.AreEqual(_companyRequest.TwoFactorBuAdmin, actualSecurity.TwoFactorBuAdmin, 
                "TwoFactorBuAdmin does not match.");
            Assert.AreEqual(_companyRequest.TwoFactorTeamAdmin, actualSecurity.TwoFactorTeamAdmin, 
                "TwoFactorTeamAdmin does not match.");
            Assert.AreEqual(_companyRequest.LogoutUrl, actualSecurity.LogoutUrl, 
                "LogoutUrl does not match.");
            Assert.AreEqual(_companyRequest.AuditTrailRetentionPeriod, 
