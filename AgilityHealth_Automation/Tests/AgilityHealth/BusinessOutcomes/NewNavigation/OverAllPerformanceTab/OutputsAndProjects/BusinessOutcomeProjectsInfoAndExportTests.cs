    ...
    OutputsAndProjects
{
    ...
    public class BusinessOutcomeProjectsInfoAndExportTests : BusinessOutcomesBaseTest
    {
        private static BusinessOutcomeResponse _projectCardResponse;
        private static List<BusinessOutcomeTagResponse> _project;
        ...
        public static void ClassSetUp(TestContext _)
        {
            ...
            _project = labels.First(a => a.Name == BusinessOutcomesCardTypeTags.ProjectsTimeline.GetDescription()).Tags;
            _projectCardResponse = CreateBusinessOutcome(SwimlaneType.Projects, 0, _project);
        }

        [TestMethod]
        [TestCategory("Critical")]
        [TestCategory("CompanyAdmin")]
        public void BusinessOutcomes_OverallPerformance_OutputsProjects_Projects_GridViewHeadersAndData()
        {
            ...
            Log.Info("Verifying 'Projects' timeline header information.");
            businessOutcomesOverallPerformancePage.ClickOnProjectTimeline(User.CompanyName, "Projects");
            Assert.IsTrue(businessOutcomesOverallPerformancePage.IsProjectHeaderDisplayed("Projects"), "'Project' Header is not displayed");
            ...
            Log.Info("Validating Filter options.");
            var expectedFilterValue = _project.Select(a => a.Name).ToList();
            businessOutcomesOverallPerformancePage.ClickOnFilterButton();
            CollectionAssert.AreEqual(expectedFilterValue, businessOutcomesOverallPerformancePage.GetFilterOptions(), "Mismatch in Filter options");

            Log.Info("Verifying Projects Grid headers.");
            var expectedProjectHeaders = new List<string> { "ID", "Title", "Card Type", "Progress", "Team", "Owners", "Status", "Start Date", "End Date" };
            CollectionAssert.AreEqual(expectedProjectHeaders, businessOutcomesOverallPerformancePage.GetProjectHeaderData(), "Mismatch in Projects Grid headers");
            businessOutcomesOverallPerformancePage.ClickOnFilterOkButton();
            businessOutcomesOverallPerformancePage.InitiativeSortByDescending("ID");

            Log.Info("Validating project data in the grid.");
            Assert.IsTrue(businessOutcomesOverallPerformancePage.IsTitleDisplayed(_projectCardResponse.Title), $"Project title '{_projectCardResponse.Title}' is not displayed");
            Assert.AreEqual("Not Started", businessOutcomesOverallPerformancePage.GetStatusValue(_projectCardResponse.Title), $"Status value mismatch for project '{_projectCardResponse.Title}'");
        }

        [TestMethod]
        [TestCategory("Critical")]
        [TestCategory("CompanyAdmin")]
        public void BusinessOutcomes_OverallPerformance_OutputsProjects_Projects_ExportToExcel()
        {
            var loginPage = new LoginPage(Driver, Log);
            var dashboardPage = new BusinessOutcomesDashboardPage(Driver, Log);
            var overallPerformancePage = new BusinessOutcomesOverallPerformancePage(Driver, Log);
            ...
            Log.Info("Clicking on the Projects timeline.");
            businessOutcomesOverallPerformancePage.ClickOnProjectTimeline(User.CompanyName, "Projects");
            ...
            Log.Info($"Checking if the {_projectCardResponse.Title} title is displayed in the exported Excel file.");
            var titles = excelDataTable.AsEnumerable().Select(row => row["Title"].ToString()).ToList();
            Assert.IsTrue(titles.Contains(_projectCardResponse.Title), $"The created card with title '{_projectCardResponse.Title}' is not displayed in the exported Excel file.");
        }
    }
}
